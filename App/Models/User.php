...
class User extends Model
{
    ...
    public static function update(
        ...
        if (Auth::isLogged() && $_SESSION['id'] > 0){
            ...
            $nameTest = unaccent($name);
            $surnameTest = unaccent($name);
            $locationTest = unaccent($name);

            if (strlen($nameTest) > 0){
                if(!preg_match("/^([a-zA-Z' ]+)$/",$nameTest)){
                    return false;
                }
            }

            if (strlen($surnameTest) > 0){
                if(!preg_match("/^([a-zA-Z' ]+)$/",$surnameTest)){
                    return false;
                }
            }

            if (strlen($locationTest) > 0){
                if(!preg_match("/^([a-zA-Z' ]+)$/",$locationTest)){
                    return false;
                }
            }
